    ...
    if (_rm.rawData) {
      ...
      applied.phases.forEach((phase, pi) => {
        ...
        phase.tasks.forEach(task => {
          ...
          tasks.push({ project: 'Arconte Retail', phase: phase.title, task, done, key, type: 'retail', isBlocked, blockedBy });
        });
      });
    }
  ...
  function refreshWeekView() {
    ...
    sprints.forEach((sprint, si) => {
      ...
      sortedPeople.forEach((person, pi) => {
        ...
        tlist.forEach(({ task, done, key, type, pid, phaseId, sprintName, planIdx, project, phase }) => {
          ...
          const cbStyle    = (!isAdmin() && !done) ? 'opacity:.35;cursor:not-allowed;' : '';
          const _tlEsc = taskLabel.replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/\n/g,' ');
          const uploadBtn  = `<button style="flex-shrink:0;cursor:pointer;width:18px;height:18px;display:flex;align-items:center;justify-content:center;border-radius:4px;background:rgba(74,222,128,${hasEvidence ? '0.18' : '0.07'});font-size:11px;line-height:1;border:1px solid rgba(74,222,128,${hasEvidence ? '0.4' : '0.15'});color:inherit;" title="Agregar evidencia / comentario" onclick="openWkCloseModal('${_tlEsc}','${key}','${type}','${pid||''}','${phaseId||''}','${taskId}',${planIdx||0},'${sprintName||''}','evidence')">📎</button>`;
          html += `<div class="wk-task${done ? ' done-wk' : ''}" id="wk-${key}">
              <input type="checkbox" ${done ? 'checked' : ''} ${cbDisabled} onchange="${extra}" style="${cbStyle}">
              <div style="flex:1;">${taskLabel}${taskSub}${shiftBadge}${phaseBadge}${solBadge}${doneNote}</div>
              ${shiftBtns}
              ${uploadBtn}
              ${type !== 'retail' && isAdmin() ? `
